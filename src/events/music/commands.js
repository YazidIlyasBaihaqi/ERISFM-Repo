...
async function music(bot, message, args, todo) {
  ...
  if (connectionMap.get(message.guildID)) {
    if (voice_channel.channelID != connectionMap.get(message.guildID).channelID) {
  ...
  switch (todo) {
                ...
                if (interaction.message.id == m.id) {
                  if (interaction.member.id == message.member.id) {
                    ...
                    return bot.createMessage(message.channel.id, {
                      embeds: [{
                        title: "**Selected**",
                        description: `[${videoResult.videos[interaction.data.custom_id].title}](${videoResult.videos[interaction.data.custom_id].url
                          }) to queue`
                      }],
                      components: [],
                    })
                      .then(resolve(video(videoResult.videos[interaction.data.custom_id])))
                      .catch(console.error);
                  } else {
    ...
    case "skip":
      if (!connectionMap.get(message.guildID)) {
        return bot.createMessage(message.channel.id, "There is no song currently playing");
      }

      if (message.member.voiceState.channelID != connectionMap.get(message.guildID).channelID) {
        return bot.createMessage(message.channel.id, "Please join the current voice channel")
      }

      connectionMap.get(message.guildID).stopPlaying();
      bot.createMessage(message.channel.id, `Skipping ${database_queue.songs[0].title}`);
      return
    case "stop":
      if (!connectionMap.get(message.guildID)) {
        return bot.createMessage(message.channel.id, "There is no song currently playing");
      ...
      bot.createMessage(message.channel.id, `Stopping the queue, deleting whole queue`);
      await SQueue.findOneAndUpdate(
        {
          guildID: message.guildID,
        },
...
async function video_player(bot, guild, message) {
  ...
  bot.joinVoiceChannel(message.member.voiceState.channelID).catch((err) => { // Join the user's voice channel
    bot.createMessage(message.channel.id, "Error joining voice channel: " + err.message); // Notify the user if there is an error
    console.log(err); // Log the error
  }).then((connection) => {
    try {
      ytdl(database_queue.songs[0].url, { filter: function (format) { return format.container === 'mp4' && !format.encoding; }, quality: 'lowest' }).pipe(fs.createWriteStream(path.join(__dirname, `./${message.guildID}.mp3`)).on('finish', () => {
